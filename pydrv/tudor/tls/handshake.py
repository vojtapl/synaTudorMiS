...
class TlsHandshakeProtocol:
    ...
    def in_handshake(self) -> bool:
        return (
            self.phase != TlsHandshakePhase.IDLE
            and self.phase != TlsHandshakePhase.FINISHED
        )
    ...
    def start_handshake(self):
        ...
        ses_id = (
            ...
            if len(self.session.session_id) == 7
            else data.TlsSessionId(bytes(7))
        )
        ...
        for s in self.cipher_suites:
            extensions += s.advertise_extensions()
        for m in self.compression_methods:
            extensions += m.advertise_extensions()
        ...
        for ext in extensions:
            for e2 in extensions_no_dupl:
                if ext.data_type.ext_type == e2.data_type.ext_type:
                    break
            else:
                extensions_no_dupl.append(ext)

        # Create ClientHello
        self.client_random = data.TlsRandom.create()
        client_hello = data.TlsHandshakeClientHello(
            data.TlsProtocolVersion.current,
            self.client_random,
            ses_id,
            cipher_suites,
            compr_methods,
    ...
    def receive(self, msg: data.TlsHandshakeMessage):
        ...
        if isinstance(msg, data.TlsHandshakeServerHello):
            if self.phase != TlsHandshakePhase.SENT_HELLO:
                raise data.TlsAlertException(
                    data.TlsAlertDescription.UNEXPECTED_MESSAGE
                )
            ...
            if len(self.compression_methods):
                for c in self.compression_methods:
                    ...
                    raise data.TlsAlertException(
                        data.TlsAlertDescription.HANDSHAKE_FAILURE
                    )
            else:
            ...
            if self.phase != TlsHandshakePhase.SUITE_HANDSHAKE:
                raise data.TlsAlertException(
                    data.TlsAlertDescription.UNEXPECTED_MESSAGE
                )
            ...
            self.record_layer.change_cipher_spec(
                self.neg_cipher_suite.create_encryption_algo()
            )
            self.send(
                data.TlsHandshakeMessage(
                    data.TlsHandshakeFinished(
                        data.tls_prf(
                            self.record_layer.encryption_algo.master_secret,
                            "client finished",
                            self.msg_digest.digest(),
                            12,
                            self.neg_cipher_suite.hash_algo,
                        )
                    )
                )
            )
            ...
            if self.phase != TlsHandshakePhase.SERVER_DONE:
                raise data.TlsAlertException(
                    data.TlsAlertDescription.UNEXPECTED_MESSAGE
                )
            ...
            if (
                self.record_layer.remote_encryption_algo
                != self.record_layer.encryption_algo
            ):
                raise data.TlsAlertException(
                    data.TlsAlertDescription.UNEXPECTED_MESSAGE
                )
            ...
            if ver_data != msg.verify_data:
                raise data.TlsAlertException(data.TlsAlertDescription.DECRYPT_ERROR)
