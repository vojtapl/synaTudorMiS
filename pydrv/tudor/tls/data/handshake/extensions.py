...
class TlsHandshakeExtensionType:
    ...
    def __eq__(self, other: TlsHandshakeExtensionType):
        if not isinstance(other, TlsHandshakeExtensionType):
            return False
...
class TlsHandshakeExtension(TlsData):
    ...
    def __repr__(self):
        return "HandshakeExtension(type=%r data=%r)" % (self.ext_type, self.ext_data)

    @staticmethod
    def read(stream: TlsDataReadStream):
        # Read extension id & size
        ext_id, ext_size = stream.read_int(2), stream.read_int(2)
        ext_type: TlsHandshakeExtensionType = TlsHandshakeExtensionType.extensions[
            ext_id
        ]
...
class TlsHandshakeExtensionSupportedGroups(TlsData):
    ...
    def __repr__(self):
        return "supported_groups(curves=%r)" % self.curves

    @staticmethod
    def read(stream: TlsDataReadStream):
        return TlsHandshakeExtensionSupportedGroups(
            TlsVector.read(
                stream, TlsHandshakeExtensionSupportedGroups.curves_vtype
            ).elements
        )

    def write(self, stream: TlsDataWriteStream):
        TlsVector(TlsHandshakeExtensionSupportedGroups.curves_vtype, self.curves).write(
            stream
        )
...
class TlsHandshakeExtensionECPointFormats(TlsData):
    ...
    def __repr__(self):
        return "ec_point_formats(formats=%r)" % self.formats

    @staticmethod
    def read(stream: TlsDataReadStream):
        return TlsHandshakeExtensionECPointFormats(
            TlsVector.read(
                stream, TlsHandshakeExtensionECPointFormats.formats_vtype
            ).elements
        )

    def write(self, stream: TlsDataWriteStream):
        TlsVector(
            TlsHandshakeExtensionECPointFormats.formats_vtype, self.formats
        ).write(stream)
