...
class TlsHandshakeMessageType:
    ...
    def __eq__(self, other: TlsHandshakeMessageType):
        if not isinstance(other, TlsHandshakeMessageType):
            return False
...
class TlsHandshakeMessage(TlsData):
    ...
    def __repr__(self):
        return "HandshakeMessage(type=%r content=%r)" % (self.msg_type, self.content)
    ...
    def read(stream: TlsDataReadStream) -> TlsHandshakeMessage:
        ...
        if msg_id not in TlsHandshakeMessageType.msg_types:
            raise TlsAlertException(TlsAlertDescription.ILLEGAL_PARAMETER)
    ...
    @classmethod
    def convert(content: TlsDataInstance):
        return TlsHandshakeMessage(content)
