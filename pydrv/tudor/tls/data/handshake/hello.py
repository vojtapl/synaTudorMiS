...
class TlsSessionId(TlsData):
    ...
    def __eq__(self, o: TlsSessionId):
        return isinstance(o, TlsSessionId) and self.data == o.data

    def __ne__(self, o: TlsSessionId):
        return not (self == o)

    def __str__(self):
        return "SessionId(len=%d)" % len(self.data) if len(self.data) > 0 else "none"

    def __repr__(self):
        return "SessionId(data='%s')" % self.data.hex()
...
class TlsHandshakeHelloRequest(TlsData):
    ...
    @staticmethod
    def read(stream: TlsDataReadStream):
        return TlsHandshakeHelloRequest()
...
class TlsHandshakeClientHello(TlsData):
    ...
    compr_methods_vtype = TlsVectorType(
        TlsCompressionMethodId, min_size=0, max_size=0xFF
    )  # BROKEN The windows driver doesn't advertise any compression methods

    def __init__(
        self,
        proto_ver: TlsProtocolVersion,
        rand: TlsRandom,
        ses_id: TlsSessionId,
        cipher_suites: list,
        compr_methods: list,
    ...
    def __repr__(self):
        return (
            "ClientHello(ver=%r rand=%r ses_id=%r cipher_suites=%r compr_methods=%r extensions=%r)"
            % (
                self.proto_ver,
                self.rand,
                self.ses_id,
    ...
    def read(stream: TlsDataReadStream):
        ...
        cipher_suites = TlsVector.read(
            stream, TlsHandshakeClientHello.cipher_suites_vtype
        ).elements
        compr_methods = TlsVector.read(
            stream, TlsHandshakeClientHello.compr_methods_vtype
        ).elements

        # Read extensions
        exts = []
        while len(stream) > 0:
            exts.append(TlsHandshakeExtension.read(stream).ext_data)

        return TlsHandshakeClientHello(
            proto_ver, rand, ses_id, cipher_suites, compr_methods, exts
        )
    ...
    def write(self, stream: TlsDataWriteStream):
        ...
        TlsVector(
            TlsHandshakeClientHello.cipher_suites_vtype, self.cipher_suites
        ).write(stream)
        TlsVector(
            TlsHandshakeClientHello.compr_methods_vtype, self.compr_methods
        ).write(stream)
        for e in self.extensions:
            TlsHandshakeExtension(e).write(stream)
...
class TlsHandshakeServerHello(TlsData):
    ...
    def __init__(
        ...
        compr_method: TlsCompressionMethodId,
        extensions: list,
    ):
        super().__init__()
        self.proto_ver = proto_ver
    ...
    def __repr__(self):
        return (
            "ServerHello(ver=%r rand=%r ses_id=%r cipher_suite=%r compr_method=%r extensions=%r)"
            % (
                self.proto_ver,
                self.rand,
                self.ses_id,
    ...
    def read(stream: TlsDataReadStream):
        ...
        return TlsHandshakeServerHello(
            proto_ver, rand, ses_id, cipher_suite, compr_method, exts
        )
...
class TlsHandshakeServerHelloDone(TlsData):
    ...
    @staticmethod
    def read(stream: TlsDataReadStream):
        return TlsHandshakeServerHelloDone()
