...
class TlsHandshakeCertificate(TlsData):
    ...
    def __repr__(self):
        return "Certificate(cert=%r)" % self.cert
...
class TlsHandshakeCertificateRequest(TlsData):
    ...
    def __repr__(self):
        return "CertificateRequest(types=%r)" % self.cert_types
    ...
    def write(self, stream: TlsDataWriteStream):
        TlsVector(TlsHandshakeCertificateRequest.cert_type_vec, self.cert_types).write(
            stream
        )
...
class TlsHandshakeCertificateVerify(TlsData):
    ...
    def __repr__(self):
        return "CertificateVerify(signed_hash=%s)" % self.signed_hash.hex()

    @staticmethod
    def read(stream: TlsDataReadStream):
        return TlsHandshakeCertificateVerify(stream.read(len(stream)))

    def write(self, stream: TlsDataWriteStream):
        stream.write(self.signed_hash)
