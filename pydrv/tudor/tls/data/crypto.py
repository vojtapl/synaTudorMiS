...
class TlsRandom(TlsData):
    ...
    def __repr__(self):
        return "TlsRandom(time=%d rbytes=%s)" % (self.unix_time, self.rand_bytes.hex())
...
class TlsCertificate(TlsData):
    ...
    def __repr__(self):
        return "TlsCertificate(data=%s)" % self.data.hex()
...
class TlsCipherSuiteId(TlsData):
    ...
    def __eq__(self, other: TlsCipherSuiteId):
        if not isinstance(other, TlsCipherSuiteId):
            return False
        return self.suite_id == other.suite_id

    def __hash__(self):
        return hash(self.suite_id)

    def __repr__(self):
        return "CipherSuiteID(id=0x%04x name=%s)" % (
            self.suite_id,
            "None" if self.suite is None else self.suite.__name__,
        )
    ...
    def read(stream: TlsDataReadStream):
        ...
        return (
            TlsCipherSuiteId.suites[suite_id]
            if suite_id in TlsCipherSuiteId.suites
            else TlsCipherSuiteId(suite_id)
        )
...
class TlsCompressionMethodId(TlsData):
    ...
    def __eq__(self, other: TlsCompressionMethodId):
        if not isinstance(other, TlsCompressionMethodId):
            return False
        return self.method_id == other.method_id

    def __hash__(self):
        return hash(self.method_id)

    def __repr__(self):
        return "CompressionMethodID(id=0x%02x name=%s)" % (
            self.method_id,
            "None" if self.method is None else self.method.__name__,
        )
    ...
    def read(stream: TlsDataReadStream):
        ...
        return (
            TlsCompressionMethodId.methods[method_id]
            if method_id in TlsCompressionMethodId.methods
            else TlsCompressionMethodId(method_id)
        )
