...
class TlsDataType:
    def read(self, stream: TlsDataReadStream) -> TlsDataInstance:
...
class TlsData(TlsDataInstance):
    ...
    def __init__(self):
        self.data_type = self.__class__

    @staticmethod
    def read(stream: TlsDataReadStream) -> TlsData:
        raise NotImplementedError()
...
class TlsIntegerType(TlsDataType):
    ...
    def __repr__(self):
        return "Integer(size=%d)" % self.size
...
class TlsVectorType(TlsDataType):
    def __init__(self, inner_type: TlsDataType, **kwargs):
        ...
        if self.fixed_size != None:
            assert self.min_size == None and self.max_size == None
        else:
            assert self.max_size != None
            self.size_size = self.max_size.bit_length() // 8 + (
                self.max_size.bit_length() % 8 > 0
            )

    def __repr__(self):
        s = "Vector(inner=%r" % self.inner_type
        if self.fixed_size != None:
            s += " size=0x%x" % self.fixed_size
        if self.min_size != None:
            s += " min_size=0x%x" % self.min_size
        if self.max_size != None:
            s += " max_size=0x%x" % self.max_size
        return s + ")"

    def read(self, stream: TlsDataReadStream):
        return TlsVector.read(stream, self)
...
class TlsVector(TlsDataInstance):
    ...
    def read(stream: TlsDataReadStream, vec_type: TlsVectorType):
        ...
        size = (
            vec_type.fixed_size
            if vec_type.fixed_size != None
            else stream.read_int(vec_type.size_size)
        )
        if vec_type.min_size != None:
            assert vec_type.min_size <= size
        if vec_type.max_size != None:
            assert size <= vec_type.max_size
        ...
        elements = []
        while len(estream) > 0:
            elements.append(vec_type.inner_type.read(estream))
    ...
    def write(self, stream: TlsDataWriteStream):
        ...
        # Write elements size
        if self.data_type.fixed_size != None:
            assert len(estream) == self.data_type.fixed_size
        if self.data_type.min_size != None:
            assert self.data_type.min_size <= len(estream)
        if self.data_type.max_size != None:
            assert len(estream) <= self.data_type.max_size
