...
class TlsRecordLayer:
    ...
    def close(self):
        ...
        self.send(
            data.TlsAlert(
                data.TlsAlertLevel.WARNING, data.TlsAlertDescription.CLOSE_NOTIFY
            )
        )
    ...
    def receive_data(self, stream: data.TlsDataReadStream) -> data.TlsDataWriteStream:
        ...
        while len(stream) > 0:
            ...
            if ptext.proto_ver != data.TlsProtocolVersion.current:
            ...
            while len(fstream) > 0:
                ...
                if ptext.content_type == data.TlsChangeCipherSpec.content_type:
                    ...
                    if alert.descr == data.TlsAlertDescription.CLOSE_NOTIFY:
                        if self.send_closed:
                            logging.log(tudor.LOG_TLS, "Remote confirmed session close")
                        else:
                            self.close()
                    ...
                    if alert.level == data.TlsAlertLevel.WARNING:
                        logging.log(
                            tudor.LOG_WARN, "Received TLS warning alert: %r" % alert
                        )
                    else:
                        self.close()
    ...
    def send(self, content: data.TlsDataInstance):
        ...
        if (
            self.content_buffer_type != None
            and self.content_buffer_type != content.data_type.content_type
        ):
            self.flush_content_buffer()
        ...
        if (
            isinstance(content, data.TlsAlert)
            and content.level == data.TlsAlertLevel.FATAL
        ):
            self.send_closed = True

    def has_data(self):
        return len(self.send_buffer) > 0 or len(self.content_buffer) > 0
    ...
    def flush_content_buffer(self):
        if self.content_buffer_type != None:
            ...
            ctext = self.plain_to_ciphertext(
                data.TlsPlaintext(
                    self.content_buffer_type,
    ...
    def plain_to_ciphertext(self, pt: data.TlsPlaintext):
        return self.encryption_algo.encrypt(self.compr_method.compress(pt))

    def cipher_to_plaintext(self, ct: data.TlsCiphertext):
        return self.compr_method.uncompress(self.remote_encryption_algo.decrypt(ct))
