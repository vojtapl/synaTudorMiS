...
class TlsNullEncryptionAlgorithm(TlsEncryptionAlgorithm):
    ...
    def encrypt(self, compr: data.TlsCompressed) -> data.TlsCiphertext:
        return data.TlsCiphertext(compr.content_type, compr.proto_ver, compr.fragment)

    def decrypt(self, ctext: data.TlsCiphertext) -> data.TlsCompressed:
        return data.TlsCompressed(ctext.content_type, ctext.proto_ver, ctext.fragment)
