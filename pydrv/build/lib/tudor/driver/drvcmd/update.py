...
class CmdUpdate(Command):
    ...
    def run(self, ctx: CmdContext, args: list):
        ...
        if args[0].lower() == "builtin":
            ctx.sensor.bootloader.update_firmware(
                tudor.sensor.SensorFirmwareUpdate.load_builtin()
            )
            print("Successfully updated to the Windows driver's builtin firmware")
        elif args[0].lower() == "fwdir":
            if len(args) <= 1:
                raise Exception("No firmware folder specified!")
            ctx.sensor.bootloader.update_firmware(
                tudor.sensor.SensorFirmwareUpdate.load_dir(" ".join(args[1]))
            )
            print("Successfully updated to specified firmware")
        elif args[0].lower() == "ufiles":
            if len(args) <= 1:
                raise Exception("At least one update file needs to be specified!")
            ...
            ctx.sensor.bootloader.update_firmware(
                tudor.sensor.SensorFirmwareUpdate.load(udata)
            )
