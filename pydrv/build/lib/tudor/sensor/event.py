...
class SensorEventType(enum.IntEnum):
    NO_EVENTS = 0
    EV_FINGER_DOWN = 1  # Windows: 0x0080
    EV_FINGER_UP = 2  # Windows: 0x0100
    EV_3 = 3  # Windows: 0x0001
    EV_4 = 4  # Windows: 0x0002
    EV_5 = 5  # Windows: 0x0040
    EV_6 = 6  # Windows: 0x0004
    EV_7 = 7  # Windows: 0x0008
    EV_8 = 8  # Windows: 0x0010
    EV_9 = 9  # Windows: 0x0020
    EV_FRAME_READY = 24  # Windows: 0x1000
...
class SensorEvent:
    ...
    def __repr__(self):
        return "SensorEvent(type=%r)" % self.event_type
...
class SensorEventHandler:
    ...
    def set_event_mask(self, events: list):
        ...
        logging.log(
            tudor.LOG_PROTO,
            "    current event sequence number: %04x" % self.event_seq_num,
        )
    ...
    def read_events(self, block: bool = False) -> int:
        if self.num_pending <= 0 and block:
            ...
            while True:
                ...
                (sensor_seq_num,) = struct.unpack(
                    "<xxxxxxB", self.sensor.comm.get_event_data()
                )
                sensor_seq_num &= 0x1F
                logging.log(
                    tudor.LOG_PROTO,
                    "    event sequence numbers: host=%d sensor=%d"
                    % (self.event_seq_num & 0x1F, sensor_seq_num),
                )
                if sensor_seq_num != self.event_seq_num:
                    break
        ...
        while True:
            try:
                ...
                req = (
                    struct.pack(
                        "<BHH", tudor.Command.EVENT_READ, self.event_seq_num, 32
                    )
                    if self.legacy_mode
                    else struct.pack(
                        "<BHHI", tudor.Command.EVENT_READ, self.event_seq_num, 32, 1
                    )
                )
                ...
                for i in range(num_evts):
                    (evt_type,) = struct.unpack(
                        "<Bxxxxxxxxxxx", resp[6 + i * 12 : 6 + (i + 1) * 12]
                    )
                ...
                logging.log(
                    tudor.LOG_PROTO,
                    "    new event sequence number: 0x%04x" % self.event_seq_num,
                )
                ...
                if self.legacy_mode:
                    raise e
                if e.status < 0x405 or 0x407 < e.status:
                    raise e
    ...
    def iter_events(self, block: bool = False):
        while True:
            ...
            if not block and len(self.event_queue) <= 0:
                return
    ...
    def wait_for_events_no_set(self, event_types: list):
        not_received_events = set(event_types)

        # Wait until we get a event which is in our types list
        for e in self.iter_events(block=True):
            if e.event_type in not_received_events:
                not_received_events.remove(e.event_type)
                if len(not_received_events) == 0:
                    return
            logging.log(tudor.LOG_WARN, "Ignoring sensor event %r" % e)
