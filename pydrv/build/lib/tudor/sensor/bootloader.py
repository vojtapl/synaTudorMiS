...
class SensorBootloaderMFWPatch:
    ...
    def __str__(self):
        return "MFWPatch(ver_major=%d ver_minor=%d ver_build=%d)" % (
            self.ver_major,
            self.ver_minor,
            self.ver_build,
        )
    ...
    def should_apply(self, sensor: Sensor) -> bool:
        return (
            sensor.fw_major == self.ver_major
            and sensor.fw_minor == self.ver_minor
            and sensor.fw_build_num < self.ver_build
        )
...
class SensorBootloaderIOTAPatch:
    ...
    def __str__(self):
        return "IOTAPatch(ver_major=%d ver_minor=%d ver_ref=%d)" % (
            self.ver_major,
            self.ver_minor,
            self.ver_rev,
        )
    ...
    def should_apply(self, sensor: Sensor) -> bool:
        return (
            sensor.cfg_ver.major == self.ver_major
            and (self.ver_minor == 0 or sensor.cfg_ver.minor == self.ver_minor)
            and sensor.cfg_ver.revision < self.ver_rev
        )
    ...
    def load(update_data: list) -> SensorFirmwareUpdate:
        ...
        for u in update_data:
            ...
            if 1 in cont:
                ...
                sth, ver_build, ver_major, ver_minor = struct.unpack(
                    "<IxxxxxxxxIBBxxxxxx", cont[1][:0x18]
                )
                assert sth == 0x41
                ...
                while len(pdata) > 0:
                    ...
                    patches.append(
                        SensorBootloaderMFWPatch(
                            ver_major, ver_minor, ver_build, pdata[4 : 4 + patch_size]
                        )
                    )
            ...
            if 3 in cont:
                ...
                sth, ver_major, ver_minor, ver_rev = struct.unpack(
                    "<IIIHxx", cont[3][:0x10]
                )
                assert sth == 0x41
                ...
                while len(pdata) > 0:
                    (patch_size,) = struct.unpack("<I", pdata[:4])
                    patches.append(
                        SensorBootloaderIOTAPatch(
                            ver_major, ver_minor, ver_rev, pdata[4 : 4 + patch_size]
                        )
                    )
                    pdata = pdata[4 + patch_size :]
    ...
    def load_dir(path: str):
        ...
        for fn in glob.iglob("%s/*.tupd" % path):
            with open(fn, "rb") as f:
                update_data.append(f.read())
        return SensorFirmwareUpdate.load(update_data)

    @staticmethod
    def load_builtin():
        return SensorFirmwareUpdate.load_dir(
            str(pathlib.Path(__file__).parent) + "/builtin_fw"
        )
...
class SensorBootloader:
    ...
    def enter_bootloader_mode(self):
        ...
        if self.sensor.initialized:
            self.sensor.uninitialize()
        self.sensor.comm.write_dft(bytes([0, 0, 0, 0, 0, 0, 1, 0]))
        self.sensor.reset()

    def exit_bootloader_mode(self):
        if not self.sensor.in_bootloader_mode():
            return
            ...
            for p in update.patches:
                if isinstance(p, SensorBootloaderMFWPatch):
                    if bypass_checks or p.should_apply(self.sensor):
                        # Apply patch
                        logging.log(
                            tudor.LOG_DETAIL, "    Applying MFW patch %s..." % p
                        )
                        self.sensor.comm.send_command(
                            struct.pack("<B", tudor.Command.BOOTLOADER_PATCH) + p.data,
                            2,
                            timeout=12000,
                        )
                    else:
                        logging.log(tudor.LOG_DETAIL, "    Skipping MFW patch %s" % p)
        finally:
            self.exit_bootloader_mode()
            ...
            for p in update.patches:
                if isinstance(p, SensorBootloaderIOTAPatch):
                    if p.should_apply(self.sensor):
                        # Apply patch
                        logging.log(
                            tudor.LOG_DETAIL, "    Applying IOTA patch %s..." % p
                        )
                        self.sensor.comm.send_command(
                            struct.pack("<B", tudor.Command.BOOTLOADER_PATCH) + p.data,
                            2,
                        ...
                        logging.log(tudor.LOG_DETAIL, "    Skipping IOTA patch %s" % p)
        finally:
            self.exit_bootloader_mode()
    ...
    def create_partition(self):
        ...
        with open(
            str(pathlib.Path(__file__).parent) + "/builtin_fw/partition.tptch", "rb"
        ) as f:
            part_patch = f.read()
        ...
        try:
            self.sensor.comm.send_command(
                struct.pack("<B", tudor.Command.BOOTLOADER_PATCH) + part_patch, 2
            )
        finally:
            self.exit_bootloader_mode()
