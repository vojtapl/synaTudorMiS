...
class TlsProtocolVersion(TlsData):
    ...
    def __eq__(self, other: TlsProtocolVersion):
        if not isinstance(other, TlsProtocolVersion):
            return False
    ...
    def __repr__(self):
        return "ProtocolVersion(%d.%d)" % (self.major, self.minor)
...
class TlsContentType:
    ...
    def __eq__(self, other: TlsContentType):
        if not isinstance(other, TlsContentType):
            return False
    ...
    def __repr__(self):
        return "ContentType(id=0x%04x name=%s)" % (self.type_id, self.data_type)
...
class TlsPlaintext(TlsData):
    ...
    @staticmethod
    def read(stream: TlsDataReadStream):
        ct_type_id, proto_ver, frag_len = (
            stream.read_int(1),
            TlsProtocolVersion.read(stream),
            stream.read_int(2),
        )
        return TlsCompressed(
            TlsContentType.types[ct_type_id], proto_ver, stream.read(frag_len)
        )
...
class TlsAlert(TlsData):
    ...
    def __repr__(self):
        return "TlsAlert(level=%r descr=%r)" % (self.level, self.descr)

    @staticmethod
    def read(stream: TlsDataReadStream):
        return TlsAlert(
            TlsAlertLevel(stream.read_int(1)), TlsAlertDescription(stream.read_int(1))
        )
