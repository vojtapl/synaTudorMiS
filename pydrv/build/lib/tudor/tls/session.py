...
def _critical_method(mthd: callable):
    def wrapper(self, *args, **kwargs):
        ...
        try:
            ...
            oerr = e
            alert = data.TlsAlert(
                data.TlsAlertLevel.FATAL, data.TlsAlertDescription.INTERNAL_ERROR
            )
        ...
        if self.record_layer != None and not self.record_layer.send_closed:
            try:
                ...
                if self.established:
                    self.record_layer.receive_data(
                        self.comm.send_command(
                            self.record_layer.flush_send_buffer(), 0x964, raw=True
                        )
                    )
                else:
                    self.record_layer.receive_data(
                        self.send_tls_data_command(
                            self.record_layer.flush_send_buffer()
                        )
                    )
            except Exception:
                logging.exception("Error while sending fatal alert")
...
class TlsSession:
    ...
    def establish(self):
        ...
        if self.established and remote_established:
            logging.log(tudor.LOG_TLS, "    host and remote already are in a session")
        if self.established and not remote_established:
            raise Exception("Host is in a session, but device isn't!")
        if not self.established and remote_established:
            raise Exception("Device is in a session, but host isn't!")
        ...
        while self.record_layer.has_data():
            self.record_layer.receive_data(
                self.send_tls_data_command(self.record_layer.flush_send_buffer())
            )
    ...
    def send_tls_data_command(self, tdata: bytes):
        return data.TlsDataReadStream(
            self.comm.send_command(
                struct.pack("<Bxxx", tudor.Command.TLS_DATA) + tdata, 0x100, raw=True
            )
        )
