...
class TlsEcdhKeyExchangeAlgorithm(TlsCipherSuite):
    ...
    def advertise_extensions(self) -> list:
        return [
            ...
            data.TlsHandshakeExtensionECPointFormats(
                [data.TlsHandshakeExtensionECPointFormats.UNCOMPRESSED]
            ),
        ]

    def start_handshake(
        self,
        handshake_proto: TlsHandshakeProtocol,
        server_hello: data.TlsHandshakeServerHello,
    ):
    ...
    def end_handshake(self):
        ...
        if (
            ...
            or not data.TlsCertificateType.ECDSA_SIGN in self.cert_request.cert_types
        ):
        ...
        TlsECPoint(
            eph_priv_key.public_key().public_numbers().x,
            eph_priv_key.public_key().public_numbers().y,
        ).write(kstream)
        self.handshake_proto.send(
            data.TlsHandshakeMessage(data.TlsHandshakeClientKeyExchange(kstream.data))
        )
...
class TlsEccAes256CbcSha1CipherSuite(TlsEcdhKeyExchangeAlgorithm):
    ...
    def create_encryption_algo(self) -> TlsEncryptionAlgorithm:
        ...
        return TlsBlockEncryptionAlgorithm(
            self.premaster_secret,
            self.handshake_proto.client_random,
            self.handshake_proto.server_random,
...
class TlsEccAes256GcmSha384CipherSuite(TlsEcdhKeyExchangeAlgorithm):
    ...
    def create_encryption_algo(self) -> TlsEncryptionAlgorithm:
        assert not self.in_handshake
        assert self.premaster_secret != None
        return TlsAEADEncryptionAlgorithm(
            self.premaster_secret,
            self.handshake_proto.client_random,
            self.handshake_proto.server_random,
            cipher.algorithms.AES,
...
class TlsEccRemoteKey(TlsRemoteKey):
    def __init__(
        ...
        client_cert: tudor.sensor.SensorCertificate,
        remote_cert: tudor.sensor.SensorCertificate,
    ):
