...
class TlsEncryptionAlgorithm:
    def __init__(
        ...
        client_random: data.TlsRandom,
        server_random: data.TlsRandom,
        ...
        self.master_secret = data.tls_prf(
            premaster_secret, "master secret", rstream.data, 48, hash_algo
        )
...
class TlsBlockEncryptionAlgorithm(TlsEncryptionAlgorithm):
    def __init__(
        ...
        # Generate keys
        kstream = data.TlsDataReadStream(
            data.tls_prf(
                self.master_secret,
                "key expansion",
                self.derive_input,
                2 * hmac_key_size + 2 * encr_key_size,
                hash_algo(),
            )
        )
    ...
    def encrypt(self, compr: data.TlsCompressed) -> data.TlsCiphertext:
        ...
        encr = cipher.Cipher(
            self.encr_algo(self.encr_key), self.encr_mode(iv)
        ).encryptor()
    ...
    def decrypt(self, ctext: data.TlsCiphertext) -> data.TlsCompressed:
        ...
        if (len(istream) % len(self.decr_key)) != 0:
            raise data.TlsAlertException(data.TlsAlertDescription.BAD_RECORD_MAC)
        decr = cipher.Cipher(
            self.encr_algo(self.decr_key), self.encr_mode(iv)
        ).decryptor()
        ddata = decr.update(istream.read(len(istream))) + decr.finalize()

        # Check padding
        pad_size = ddata[-1]
        if not (0 <= pad_size < len(self.decr_key)):
            raise data.TlsAlertException(data.TlsAlertDescription.BAD_RECORD_MAC)

        # Create TLSCompressed
        h = hmac.HMAC(self.decr_mac_key, self.hash_algo())
        compr = data.TlsCompressed(
            ctext.content_type,
            ctext.proto_ver,
            ddata[: -(h.algorithm.digest_size + pad_size)],
        )
        ...
        if h.finalize() != ddata[-(h.algorithm.digest_size + pad_size) : -pad_size]:
            raise data.TlsAlertException(data.TlsAlertDescription.BAD_RECORD_MAC)
    ...
    def decrypt(self, ctext: data.TlsCiphertext) -> data.TlsCompressed:
        try:
            ...
            data.TlsCompressed(
                ctext.content_type,
                ctext.proto_ver,
                bytes(len(ctext.fragment) - 8 - self.tag_size),
            ).write(
                cdata
            )  # We just need the header
            decr = cipher.Cipher(
                self.encr_algo(self.decr_key), self.encr_mode(self.decr_iv + nonce)
            ).decryptor()
            decr.authenticate_additional_data(
                self.decr_seq_num.to_bytes(8, "big") + cdata.data[:5]
            )

            # Decrypt fragment
            dfrag = decr.update(
                istream.read(len(istream) - self.tag_size)
            ) + decr.finalize_with_tag(istream.read(self.tag_size))
